...
- **Metro를 짧은 시간에 여러 번 강제 종료/재시작하면(특히 `curl .../reload`, `am force-stop`+재실행을 반복) 번들 전달 자체가 꼬여서 앱이 "Loading from 10.0.2.2:8081..."에서 무한정 멈추는 경우가 있었음.** 원인을 앱 코드가 아니라 Metro 프로세스 상태로 오판하고 재시작을 반복하면 오히려 악화됨. **재발 방지: JS만 바뀐 경우 재시작을 연달아 하지 말고, 먼저 `curl localhost:8081/status`로 Metro가 살아있는지 확인 → 안 될 때만 딱 한 번 깨끗하게 재시작(포트 점유 프로세스 정리 → Metro 새로 띄우고 `/status`로 뜬 것 확인 → 앱은 `am force-stop` 후 1회만 콜드 스타트) → 그래도 안 되면 재시도하지 말고 멈추고 logcat(`adb logcat -d --pid=$(adb shell pidof <package>)`)으로 실제 원인부터 확인할 것.**
  - 이 문제를 logcat으로 파봤을 때 `ReactNativeJS`/`BundleDownloader` 쪽에서 `java.net.ProtocolException: Expected leading [0-9a-fA-F] character but was 0xd` (multipart 번들 응답의 chunked encoding이 에뮬레이터의 10.0.2.2 NAT 경로를 지나면서 깨지는 것으로 보임)가 반복적으로 찍힌 사례가 있었음. Metro 자체(`/status`)는 정상, `curl`로 호스트에서 직접 번들을 받으면 200 정상 — 에뮬레이터 쪽 요청만 실패.
